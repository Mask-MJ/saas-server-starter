...
import { ValveEntity, ValveOperatingDataEntity } from './valve.entity';
...
export class ValveController {
   ...
   */
  @Get('operating-data')
  @ApiOkResponse({ type: ValveOperatingDataEntity, isArray: true })
  @Permissions('project:valve:query')
  findOperatingDataList(@Query() queryValveListDto: QueryValveListDto) {
    return this.valveService.findOperatingDataList(queryValveListDto);
  }

  /**
   * 获取阀门运行数据详情
   */
  @Get('operating-data/:id')
  @Permissions('project:valve:query')
  findOperatingData(@Param('id') id: number) {
    return this.valveService.findOperatingData(id);
  }
